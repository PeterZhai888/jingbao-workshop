...
[{"shotNumber":1,"sceneDescription":"中景：人物站在书架前面对镜头，右手举起一本书，眉头微皱，语气认真","dialogue":"如果你每天都很努力，却还是赚不到钱……","duration":"3秒","cameraMove":"固定机位，手机与胸同高"}]`,
    ...
    // 🤖 AI视频：核心是"每条描述自包含锚点"——用户会把单个镜头单独复制进AI视频工具，工具看不到其他镜头
...
3. 每个镜头包含以下字段：
   - shotNumber: 镜头序号（从 1 开始的整数）
   - sceneDescription: 画面描述（中文，60-120字，按需写足信息即可、不凑字数；无需考虑拍摄成本，可写现实中无法实拍的画面）
   - dialogue: 台词或旁白（可为空字符串；有台词时标注说话人，如"旁白：""女主："）
   - duration: 预估时长（如"3秒"、"5秒"）
   - cameraMove: 运镜建议（如"缓慢推镜，全景→中景"、"环绕运镜"、"航拍俯瞰"、"跟拍"）
4. 跨镜头连续性（最重要）：每个镜头会被用户单独复制到AI视频工具生成，工具看不到其他镜头，因此每条 sceneDescription 必须自包含关键锚点，不能依赖"读者记得上一镜"：
   - 风格锚点：全片统一视觉风格短语（用户文案指定了就沿用，如"3D动画风格"；未指定则根据剧情自选一种），每个镜头描述以它开头，禁止风格漂移
   - 人物锚点：人物首次出场写清年龄段、发型发色、核心服装，压缩成一个固定短语（如"25岁黑短发灰卫衣男生"），此后每个镜头原样复用该短语、禁止换同义词改写；多人同框时双方锚点都要写
   - 场景锚点：每个场景起固定名称（如"暖光客厅"），首次出现时写清1-2个关键空间锚点（如"白色单扇木门位于右侧，餐桌靠窗"）与光线色调，同场景后续镜头复用场景名并保持空间关系、光线色调一致
   - 道具与状态锚点：可数道具写具体数量（如"四菜一汤"，禁止"一桌饭菜"式模糊表述）；门的开/关、人物的坐/站、手中物品等状态在后续镜头中显式继承，不得无故消失或复位
   - 新人物首次出现：允许剧情需要的突然登场，但必须当场建立其人物锚点并写明空间位置与动作，禁止凭空出现且无描述
5. 动作密度与镜头变化：每个镜头只安排 1-2 个核心动作（AI视频在有限时长内无法自然完成连续多动作）；固定"世界"、自由"摄影"——上述锚点保持稳定，但景别、机位、构图、运镜、人物动作与情绪自由变化，相邻镜头画面不得雷同
6. 连续性例外：剧情明确要求变化（换装、门被关上、道具被拿走、时间跳跃如"次日清晨"）时允许改变锚点，但必须在描述中明确写出变化本身，不得让变化看起来像随机出错
7. 画面描述向AI生成提示词靠拢：具体的光线（晨光/冷色/逆光）、氛围（史诗感/治愈/悬疑）、构图细节，便于直接用于AI生图或生视频工具
示例输出格式（注意镜头1建立全部锚点，镜头2、3原样复用锚点短语，同时变化景别与动作）：
[{"shotNumber":1,"sceneDescription":"3D动画风格，暖光客厅，白色单扇木门位于画面右侧，靠窗餐桌摆着四菜一汤，25岁黑短发灰卫衣男生提着公文包站在门口，神情疲惫，暖黄台灯光洒在他脸上","dialogue":"旁白：又是一天加班到深夜","duration":"4秒","cameraMove":"全景，缓慢推镜"},{"shotNumber":2,"sceneDescription":"3D动画风格，暖光客厅，25岁黑短发灰卫衣男生推开白色单扇木门走进屋内，眼中露出惊喜，身后靠窗餐桌上四菜一汤冒着热气，暖黄灯光笼罩全屋","dialogue":"","duration":"4秒","cameraMove":"中景，跟拍"},{"shotNumber":3,"sceneDescription":"3D动画风格，暖光客厅，25岁黑短发灰卫衣男生坐在靠窗餐桌旁，扎马尾米色家居服女友站在桌边为他盛汤，桌上四菜一汤热气腾腾","dialogue":"女友：快趁热吃","duration":"4秒","cameraMove":"近景，固定机位"}]`,
