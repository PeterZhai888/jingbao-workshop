...
5. 【视觉风格锚点】全片统一视觉风格短语：用户文案指定了就严格沿用（如"3D动画风格"）；未指定则根据剧情自选一种（如"冷色调2D动画风格"、"电影感写实风格"）。风格短语应简洁、固定、可重复，不堆叠多个相近、重复或互相冲突的风格标签（推荐"冷色调2D动画风格"这类短标签，禁止"高质量电影级超精细赛博朋克日系二次元动画风格"式冗长堆砌）。每个镜头描述以同一风格短语开头，禁止镜头之间风格漂移。
6. 【人物身份与锚点连续性】人物首次出现时建立固定锚点短语（年龄段+发型发色+核心服装+必要的显著特征，如"25岁黑短发灰卫衣男生"），此后每个镜头原样复用该短语，禁止换同义词改写。禁止无剧情原因改变发型、发色、核心服装、年龄感、性别或身份；剧情要求换装、受伤、淋湿、变身等变化时，必须明确写出变化原因与变化后状态。多人同框时双方锚点都要写；出现多个外观相近的人物时，禁止只用"男生""女生"等模糊称呼，并写明人物之间的位置关系与互动对象。新人物允许因剧情需要突然登场，首次出现必须当场建立固定人物锚点，并根据剧情需要说明其出现方式或来源、空间位置与当前动作（如"扎马尾米色家居服女友从厨房端汤走出，来到餐桌右侧"），避免无解释地凭空出现；镜头切换或场景建立本身已能合理解释人物在场时，无需强行补写来源。
7. 【场景与空间关系连续性】每个重要场景起固定名称（如"暖光客厅"），首次出现时写清1-2个关键空间结构（如"白色单扇木门位于右侧，餐桌靠窗"）、重要物体位置、光线与整体色调；同场景后续镜头保持门窗数量、重要家具位置、空间关系、光线色调一致，禁止无故改变。涉及跨空间移动、进出场景或位置变化影响画面理解时，写明人物从哪里来、朝哪里移动、到达哪里（如"男生从画面左侧的门口进入，向右侧餐桌走去"）；简单局部动作（如"少年缓缓后退两步"）无需机械补充完整移动路径。禁止人物刚进入一个空间，下一镜头突然出现在从未建立过的其他空间。
8. 【状态与道具连续性】上一镜头结束状态 = 下一镜头默认开始状态。对影响画面连续性、人物互动或剧情的重要可数道具，尽量写明具体数量（如"四菜一汤"、"桌上放着两只玻璃杯"），避免影响连续性的模糊表述（如"一桌饭菜"），但不要求机械统计背景物品数量。人物坐/站/蹲姿态、所在位置、朝向、手中物品、门窗开关、道具位置与去留、衣物干湿、受伤、天气与环境光线等客观状态必须严格继承，除非剧情明确描述变化，否则不得无故消失、复位或改变（如：门打开后下一镜头默认仍开着；手机未放下就不得突然出现在桌上）。人物表情与情绪默认延续，但剧情事件、人物互动、动作或台词导致变化时允许自然变化，并应体现变化原因或过程（如疲惫的人看到一桌饭菜后露出惊喜，属合理变化）。
...
14. 【生成友好】同场景、同人物、连续动作且状态变化较小的相邻镜头，应保持画面与状态自然承接，方便用户需要时连续组合生成；出现新场景、新人物、人物状态明显变化、大型特效、激烈动作、复杂多人互动或明显时间跳跃的镜头属高复杂度镜头，应进一步强化该镜头的独立性与关键信息完整度，不依赖其他镜头才能理解。无论哪种情况，单镜头独立、自包含、可单独复制生成始终是第一优先级。
15. 【镜头独立价值】每个镜头必须承担至少一种明确作用：建立环境、展示人物、推进动作、展示反应、强调关键信息、呈现冲突、展示结果、制造情绪或视觉冲击。禁止为凑镜头数量把同一动作或画面机械拆成多个高度重复的镜头；相邻镜头须在景别、构图、机位、动作、情绪、叙事功能中至少一项有明显变化。连续动作或连续状态变化的镜头可以保持摄影方式相对稳定，通过主体动作、状态或叙事推进形成镜头变化，无需为了制造差异强行改变景别、机位或运镜。
16. 【画面描述写法与优先级】描述顺序：视觉风格 → 场景与关键空间关系 → 人物锚点 → 当前动作与状态 → 光线氛围；重点明确、易于AI视频模型理解，不堆砌无关细节。当各项要求冲突时，优先级为：可生成性 > 连续性 > 画面效果 > 文学修辞。
示例输出格式（镜头1建立风格、场景、人物锚点与初始状态；镜头2演示动作链承接、人物移动与门状态变化；镜头3演示状态变化的显式体现——放下公文包后坐下，以及新人物合理登场与景别变化）：
[{"shotNumber":1,"sceneDescription":"3D动画风格，暖光客厅，关闭的白色单扇木门位于画面右侧，靠窗餐桌摆着四菜一汤，25岁黑短发灰卫衣男生提着公文包站在门前，神情疲惫，暖黄灯光从门缝透出","dialogue":"旁白：又是一天加班到深夜","duration":"4秒","cameraMove":"全景，缓慢推镜"},{"shotNumber":2,"sceneDescription":"3D动画风格，暖光客厅，25岁黑短发灰卫衣男生提着公文包推开白色单扇木门，走向靠窗餐桌，眼中露出惊喜，身后木门保持敞开，桌上四菜一汤冒着热气，暖黄灯光笼罩全屋","dialogue":"","duration":"4秒","cameraMove":"中景，跟拍"},{"shotNumber":3,"sceneDescription":"3D动画风格，暖光客厅，25岁黑短发灰卫衣男生将公文包放在脚边后坐在靠窗餐桌旁，身后白色单扇木门保持敞开，抬头望向从厨房端汤走来的扎马尾米色家居服女友，桌上四菜一汤热气腾腾","dialogue":"女友：快趁热吃","duration":"4秒","cameraMove":"近景，固定机位"}]`,
